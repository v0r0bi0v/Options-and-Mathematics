        ...
        \begin{frame}{Базовые понятия}
            \begin{randomvariable*}
                \begin{itemize}
                    ...
                    \[
                        \E \hspace{0.1cm}\xi = \int\limits_{-\infty}^{+\infty} xf_{\xi}(x) dx
                    \]
                    \item Дисперсия
                    \[
                        \D \hspace{0.1cm}\xi = \E\left[(\xi - \E\hspace{0.1cm} \xi)^2\right] \left( = \sum\limits_{i\in I} x_i \cdot \P(\xi = x_i) \right)
                    \]
                \end{itemize}
            \end{randomvariable*}
        \end{frame}
